 ...
 *                          Trails are PERMANENT for the round (classic Curve
 *                          Fever, with periodic gaps) — see trailPoints below.
 *   playerDied           { socketId, wallet, reason }
 *                          reason: 'wall' | 'trail' | 'shrink'
 *                          'wall'   — hit the arena/shrink boundary (always lethal)
 *                          'trail'  — hit a trail (own or another player's,
 *                                     anywhere it's been laid down this round)
 ...
 *                    (lengthMultiplier is kept at a constant 1.0 for frontend
 *                    backward compatibility only — classic mode has no body-
 *                    length mechanic; eliminations grant a speed bonus instead.)
 *     - trail:       gameState.players[].trailPoints[] (each [x,y,r,gapFlag]) —
 *                    LETHAL, permanent for the round, used for collision and
 *                    rendering. bodyPoints carries an identical copy of the
 *                    same array, kept only as a name-compat alias for older
 *                    frontend code. gapFlag is 1 on a dash's first/last point.
 ...
 *       lengthMultiplier,   // constant 1.0 — kept only for frontend back-compat
 *       trailPoints,        // [[x,y,r,gapFlag], ...] — LETHAL, permanent for
 *                            // the round, oldest-to-newest, decimated for network
 *       bodyPoints,         // identical alias of trailPoints (back-compat name)
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', async (reason) => {
      ...
      if (socket.data.registeredWallet) {
        await presenceService.goOffline(socket.data.registeredWallet, socket.id);
      }
    });
  });
